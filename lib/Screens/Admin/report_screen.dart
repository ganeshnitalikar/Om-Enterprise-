...
class BalanceReportForm extends StatelessWidget {
  ...
  Widget _buildDateRangePicker() {
    return Row(
      children: [
        Expanded(
            child: _buildDatePickerField('From Date', controller.fromDate)),
        SizedBox(width: 10),
      ...
      children: [
        Expanded(
          child: _buildDropdown(
              'Shop', controller.shopId, controller.shopDropdownItems),
        ),
        SizedBox(width: 1),
        Expanded(
          child: _buildDropdown('Employee', controller.driverId,
              controller.employeeDropdownItems),
        ),
        SizedBox(width: 1),
        Expanded(
  ...
  Widget _buildDropdown(
      String label, RxInt selectedId, List<DropdownItem> items) {
    return Obx(() => DropdownButtonFormField<int>(
          value: items.isNotEmpty &&
                  items.any((item) => item.id == selectedId.value)
              ? selectedId.value
              : null,
          decoration: InputDecoration(
            labelText: label,
